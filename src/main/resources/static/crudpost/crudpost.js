...
function editData() {
  ...
  if (selectedRow) {
    ...
    // Thực hiện các thao tác cần thiết để chỉnh sửa dữ liệu (ví dụ: mở một form chỉnh sửa)
    console.log("Chỉnh sửa dữ liệu cho bài đăng có ID:", selectedRowId);
  } else {
...
function fetchPosts() {
  fetch("http://localhost:8081/api/homes/")
    ...
    .catch((error) => console.error("Lỗi khi tải dữ liệu bài đăng:", error));
}
